...
exporter.parse(
  rootGroup,
  (gltfJson) => {
    const outPath = path.join(modelsDir, 'predyx_human_anatomy.gltf');
    fs.writeFileSync(outPath, JSON.stringify(gltfJson, null, 2), 'utf8');
    const stats = fs.statSync(outPath);
    console.log(`✔ Successfully generated GLTF asset: ${outPath} (${(stats.size / 1024).toFixed(1)} KB)`);
  },
  (error) => {
    console.error('Error generating GLTF asset:', error);
    process.exit(1);
  },
  { binary: false }
);
